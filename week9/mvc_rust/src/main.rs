...
#[derive(Deserialize)]
struct RegisterForm { username: String, password: String, name: String }
...
async fn register(form: web::Form<RegisterForm>, state: web::Data<AppState>) -> impl Responder {
    ...
    match res {
        Ok(_) => HttpResponse::Created().json(serde_json::json!({"username": form.username, "name": form.name})),
...
#[derive(Deserialize)]
struct ChangeProfileForm { newusername: Option<String>, newname: Option<String> }
...
async fn change_profile(session: Session, form: web::Form<ChangeProfileForm>, state: web::Data<AppState>) -> impl Responder {
    ...
    .fetch_one(&state.db).await;
    let user = match user { Ok(u) => u, Err(_) => return HttpResponse::Forbidden().json(serde_json::json!({"error": "用户不存在"})) };

    if form.newusername.is_none() && form.newname.is_none() {
        return HttpResponse::BadRequest().json(serde_json::json!({"error": "未检测到有效更改"}));
    }
    if let Some(newu) = &form.newusername { if *newu == user.username { return HttpResponse::BadRequest().json(serde_json::json!({"error": "新用户名与原用户名相同"})); } }
    if let Some(newn) = &form.newname { if *newn == user.name { return HttpResponse::BadRequest().json(serde_json::json!({"error": "新姓名与原姓名相同"})); } }
    ...
    match res {
        Ok(_) => {
            if let Some(newu) = &form.newusername { let _ = session.insert("username", newu); }
...
#[derive(Deserialize)]
struct ChangePasswordForm { oldpassword: String, password: String, password1: String }
...
async fn change_password(session: Session, form: web::Form<ChangePasswordForm>, state: web::Data<AppState>) -> impl Responder {
    ...
    .bind(username.clone().unwrap())
    .fetch_one(&state.db).await;
    let user = match user { Ok(u) => u, Err(_) => return HttpResponse::NotFound().json(serde_json::json!({"error": "用户不存在"})) };
    if !verify(&form.oldpassword, &user.password).unwrap_or(false) {
        return HttpResponse::Forbidden().json(serde_json::json!({"error": "原密码错误"}));
    }
    if form.password != form.password1 { return HttpResponse::BadRequest().json(serde_json::json!({"error": "两次输入的新密码不一致"})); }
    let hashed = match hash(&form.password, DEFAULT_COST) { Ok(h) => h, Err(_) => return HttpResponse::InternalServerError().json(serde_json::json!({"error": "密码加密失败"})) };
    let res = sqlx::query("UPDATE users SET password = ?, updated_at = NOW() WHERE username = ?")
        .bind(&hashed)
        .bind(username.unwrap())
        .execute(&state.db).await;
    match res { Ok(_) => HttpResponse::Ok().json(serde_json::json!({"message": "密码修改成功"})), Err(_) => HttpResponse::InternalServerError().json(serde_json::json!({"error": "更新用户信息失败"})) }
}

#[get("/api/admin/users")]
async fn users_data(session: Session, state: web::Data<AppState>) -> impl Responder {
    let username: Option<String> = session.get("username").unwrap_or(None);
    if username.is_none() { return HttpResponse::Unauthorized().json(serde_json::json!({"error": "未认证,请先登录"})); }
    if username.as_deref() != Some("admin") { return HttpResponse::Forbidden().json(serde_json::json!({"error": "权限不足,需要管理员权限"})); }
    let users = sqlx::query_as::<_, User>(
        "SELECT id, username, password, name, created_at, updated_at FROM users"
    )
    .fetch_all(&state.db).await;
    match users {
        Ok(list) => HttpResponse::Ok().json(serde_json::json!({"users": list, "count": list.len()})),
...
async fn main() -> std::io::Result<()> {
    ...
    let pool = MySqlPoolOptions::new().max_connections(5).connect(db_url).await.expect("数据库连接失败");

    // init templates (copy from week6/mvc/template)
    let mut tera = Tera::new("templates/**/*").expect("模板加载失败");

    let state = AppState { db: pool, tera: tera.clone() };
